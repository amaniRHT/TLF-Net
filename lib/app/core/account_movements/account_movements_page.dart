import 'package:e_loan_mobile/app/controllers.dart';
import 'package:e_loan_mobile/app/core/account_movements/accoun_movements_filter_dialog.dart';
import 'package:e_loan_mobile/app/core/account_movements/account_movements_item.dart';
import 'package:e_loan_mobile/config/config.dart';
import 'package:e_loan_mobile/widgets/widgets.dart';
import 'package:flutter/material.dart';
import 'package:get/get.dart';

class AccountMovementsPage extends StatefulWidget {
  const AccountMovementsPage({Key key}) : super(key: key);
  @override
  _State createState() => _State();
}

class _State extends State<AccountMovementsPage> {
  final AccountMovementsController controller = Get.find();

  @override
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) {
      controller.reinitialize();
    });
  }

  @override
  Widget build(BuildContext context) {
    return GetBuilder<AccountMovementsController>(
      builder: (_) {
        return SafeAreaManager(
          child: Scaffold(
            resizeToAvoidBottomInset: false,
            appBar: const CommonAppBar(),
            body: _buildContent(),
            floatingActionButton: _buildFloatingAction(),
            floatingActionButtonLocation:
                FloatingActionButtonLocation.centerFloat,
          ),
        );
      },
    );
  }

  Padding _buildContent() {
    return Padding(
      padding: AppConstants.mediumPadding,
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          const VerticalSpacing(27),
          const CommonTitle(title: 'Mouvements de votre compte'),
          const VerticalSpacing(16.0),
          _buildTotalResult(
            '${controller.totalResults} résultats obtenus',
          ),
          const VerticalSpacing(14),
          _inForceContractsList()
        ],
      ),
    );
  }

  Future<void> _refreshcontractstList() async {
    controller.getAccountMovements(fromBeginnig: true);
  }

  Padding _buildTotalResult(String title) => Padding(
        padding: const EdgeInsets.only(left: 12.0),
        child: Text(title, style: AppStyles.mediumBlue10),
      );

  Flexible _inForceContractsList() {
    return Flexible(
      child: controller.totalResults == 0 && !controller.isLoading
          ? NoDataPlaceholder(
              placeholderText:
                  controller.isLoading ? '' : AppConstants.NO_RESULTS,
              onPressed: _refreshcontractstList,
            )
          : RefreshIndicator(
              onRefresh: _refreshcontractstList,
              child: ListView.builder(
                physics: const BouncingScrollPhysics(
                    parent: const AlwaysScrollableScrollPhysics()),
                padding: const EdgeInsets.only(top: 5, bottom: 120),
                itemCount: controller.accountMovementsList.length + 1,
                itemBuilder: (BuildContext context, int index) {
                  return (index == controller.accountMovementsList.length)
                      ? PagingIndicator(
                          visiblityCondition: !controller.noMorePages &&
                              !controller.accountMovementsList.isEmpty,
                        )
                      : (index < controller.accountMovementsList.length)
                          ? AccountMovementsItem(index: index)
                          : const SizedBox();
                },
                controller: controller.scrollController,
              ),
            ),
    );
  }

  FloatActions _buildFloatingAction() {
    return FloatActions(
      toolTip: 'Imprimer la liste des mouvements de votre compte',
      iconData: Icons.print,
      filterCount: controller.filterCount.toString(),
      onAddPressed: () {
        controller.downloadBeingImplementedContractsList();
      },
      downloadButtonEnabled: true,
      showTextInfo: true,
      showMonthInfo: false,
      paddingTop: 0,
      onFilterPressed: () => {
        controller.initialiseTECs(),
        presentDialog(
          () => const AccountMovementsFilterDialog(),
        )
      },
    );
  }
}
